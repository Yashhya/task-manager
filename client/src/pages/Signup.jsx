...
const Signup = () => {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 auth-bg" style={{ background: "linear-gradient(135deg, #080c18 0%, #1a103a 35%, #0f172a 70%, #0c0f1f 100%)" }}>
      {/* Decorative orbs */}
      <div className="absolute top-1/3 right-1/4 w-72 h-72 rounded-full animate-float" style={{ background: "radial-gradient(circle, rgba(99,102,241,0.15) 0%, transparent 70%)", filter: "blur(40px)" }} />
      <div className="absolute bottom-1/4 left-1/3 w-60 h-60 rounded-full animate-float" style={{ background: "radial-gradient(circle, rgba(168,85,247,0.12) 0%, transparent 70%)", filter: "blur(40px)", animationDelay: "-3s" }} />
      ...
      <div className="w-full max-w-md animate-fade-in relative z-10">
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 text-base">Join your team workspace</p>
        </div>
        ...
        >
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              ...
              <input type="password" style={inputStyle} placeholder="Password (min 6 chars)" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required minLength={6} />
            </div>
            <div>
              <label className="block text-sm font-semibold mb-2.5 text-slate-300">Select Role</label>
              <div className="grid grid-cols-2 gap-3">
                {["member", "admin"].map((role) => (
                  <button
                    ...
                    className="py-3 rounded-xl text-sm font-semibold transition-all cursor-pointer capitalize"
                    style={{
                      background: form.role === role ? "linear-gradient(135deg, #6366f1, #8b5cf6)" : "rgba(255,255,255,0.04)",
                      color: form.role === role ? "white" : "#94a3b8",
                      border: `1px solid ${form.role === role ? "transparent" : "rgba(255,255,255,0.1)"}`,
                      boxShadow: form.role === role ? "0 4px 15px -3px rgba(99,102,241,0.4)" : "none",
                    }}
                  >
                    {role === "admin" ? "🛡️ " : "👤 "}{role}
                  </button>
